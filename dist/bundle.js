...
var Store = exports.Store = function () {
    ...
    var onBefore = {};
    var onComplete = {};
    var CACHE_KEY = 'redux-mini-cache';
    var cachedStore = null;
    var enableCaching = false;
    var shouldLoadFromCache = false;

    /**
     * Registers and creates a new data store. You are only allowed one store per application.
     *
     * @param initialData - data to initialize the store with
     * @param options - see Store Options for more information
     * @constructor
     */
    function Store(initialData, _options) {

        if (cachedStore) return cachedStore;

        var options = _options || {};
        var subscribers = options.subscribers || [];
        var data = initialData || {};
        var commands = [];
        var reduce = _reduce.bind(this);
        var firstTime = true;

        var _options$actionKey = options.actionKey,
            actionKey = _options$actionKey === undefined ? 'type' : _options$actionKey,
            _options$enableCachin = options.enableCaching,
            enableCaching = _options$enableCachin === undefined ? enableCaching : _options$enableCachin,
            shouldLoadFromCache = options.shouldLoadFromCache;
         ...
         * @param actions
         */
        this.dispatch = function (actions) {
            reduce(actions);
        };
        ...
        this.getExecutedActions = function () {
            return [].concat(_toConsumableArray(commands));
        };
        ...
        function _reduce(actions, done) {
            var _marked = /*#__PURE__*/regeneratorRuntime.mark(animationGenerator);
            ...
            var animationCallbackGenerator = animationGenerator(function () {
                setTimeout(function () {
            ...
            /**
             * Generator calls each element in the allEvents Array when the previous event has called next function
             * @param next
             */
            function animationGenerator(next) {
                var index, cb;
                return regeneratorRuntime.wrap(function animationGenerator$(_context) {
                    while (1) {
                        switch (_context.prev = _context.next) {
                            case 0:
                                _context.t0 = regeneratorRuntime.keys(allEvents);
        ...
        function logTheAction(actions) {
            ...
            commands = [].concat(_toConsumableArray(commands), _toConsumableArray(actions));
        ...
        function startup() {
            ...
            if (shouldLoadFromCache && cache) data = JSON.parse(cache);else data = _defineProperty({}, actionKey, 'INIT');
     ...
     *
     * @param propertyPath - path in the data store to save the results from the reducer
     * @param eventName -  Custom(unique) event to associate with the action
     * @param reducer - callback function that returns a non-mutated result.
     */
    Store.registerReducer = function (propertyPath, eventName, reducer) {

        var reducers = registeredReducers[eventName] || [];
    ...
    function callbackHelper(collection) {
        ...
        return function (event, cb) {
            var callbacks = collection[event] || [];
            callbacks.push(cb);
     ...
     * @param event {string} - name of event to attach the callback
     */
    ...
    /**
     * Allows the store to push data to session storage
     * @param shouldEnableCaching
     */
    Store.prototype.enableCaching = function (shouldEnableCaching) {
     ...
     * @param loadFromCache
     */
    Store.shouldLoadFromCache = function (loadFromCache) {
